...
public class Landscape {
	...
	public static Landscape createRandomHeightMap(Point3D cornerPosition, float width, float depth, int widthRes, int depthRes, float averageHeightVariation) {
		Landscape hmap = new Landscape(widthRes, depthRes);
		hmap.makeRandomHeightMap(cornerPosition, width, depth,
				averageHeightVariation);
		hmap.calculateNormals();
		hmap.loadArrays();
		return hmap;
	}

	public static Landscape createBicubicBezierPatch(Point3D[][] controlPoints,
			int widthRes, int depthRes) {
		Landscape bpatch = new Landscape(widthRes, depthRes);
	...
	private Landscape(int widthRes, int depthRes) {
	...
	private void makeRandomHeightMap(Point3D cornerPosition, float width,
			float depth, float averageHeightVariation) {
		heightValues = new float[widthRes + 1][depthRes + 1];

		float widthIncr = width / (float) widthRes;
		float depthIncr = depth / (float) depthRes;
		...
		for (int i = 0; i <= widthRes; i++) {
			for (int j = 0; j <= depthRes; j++) {
				if (i == 0) {
					if (j == 0) {
						heightValues[i][j] = rand.nextFloat();
					} else {
						heightValues[i][j] = heightValues[i][j - 1]
								+ (rand.nextFloat() - 0.5f)
								* averageHeightVariation;
					}
				} else {
					if (j == 0) {
						heightValues[i][j] = heightValues[i - 1][j]
								+ (rand.nextFloat() - 0.5f)
								* averageHeightVariation;
					} else {
						heightValues[i][j] = (heightValues[i - 1][j] + heightValues[i][j - 1])
								/ 2.0f
								+ (rand.nextFloat() - 0.5f)
								* averageHeightVariation;
					}
		...
		for (int i = 0; i <= widthRes; i++) {
			for (int j = 0; j <= depthRes; j++) {
				points[i][j] = new Point3D(cornerPosition.x + (float) i
						* widthIncr, cornerPosition.y + heightValues[i][j],
						cornerPosition.z + (float) j * depthIncr);
			}
		}
	}
